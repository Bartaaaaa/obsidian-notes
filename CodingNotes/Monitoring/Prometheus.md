...
Son architecture est basée sur un modèle de type **pull** (récupère les données dans des services (scraping), et les services peuvent récupérer des données de lui)dans lequel Prometheus récupère les données de métriques auprès des cibles qu'il surveille. Son composant principal est son serveur qui va collecter les données à intervalle régulier. Ses targets sont des app, services, serveurs, conteneurs,...
