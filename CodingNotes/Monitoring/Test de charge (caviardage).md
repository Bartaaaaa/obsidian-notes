...
**InfluxDB** est une BDD TSDB (time series database), une bdd optimisée pour stocker des données horodatées (liées au temps). Pendant le test de charge, k6 génère des milliers de métriques à la seconde. Il faut un endroit ultra rapide pour écrire ces données en temps réel. C'est le rôle d'influxDB.
