...
`package.json`Il contient la liste des dépendances requises pour le projet, ainsi que leurs plages de versions souhaitées, spécifiées à l'aide du versionnage sémantique ou de numéros de version spécifiques.

`package-lock.json`Il contient les versions résolues de toutes les dépendances, leurs sous-dépendances et leurs emplacements d'installation précis. Il constitue un instantané de l'arborescence des dépendances, garantissant ainsi des installations cohérentes.
